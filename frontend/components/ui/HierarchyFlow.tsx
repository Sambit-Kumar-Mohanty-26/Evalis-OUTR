...
import { Plus, X, ChevronRight, Pencil, Check, Building2, FolderTree, GitBranch } from "lucide-react";
...
function TreeNodeItem({ node, allNodes, isLast, onAdd, onRemove, onRename, onTypeChange, onToggle }: TreeNodeProps) {
    ...
    const commitName = () => {
        setEditing(false);
        if (nameVal.trim()) onRename(node.id, nameVal.trim());
        else setNameVal(node.name);
    };

    const commitType = () => {
        setEditingType(false);
        if (typeVal.trim()) onTypeChange(node.id, typeVal.trim());
        else setTypeVal(node.type);
    };
...
export function HierarchyFlow({ nodes, onNodesChange }: HierarchyFlowProps) {
    ...
    const addChild = (parentId: string) => {
        ...
        const initialId = crypto.randomUUID().slice(0, 8);
